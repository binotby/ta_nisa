...
if (isset($_POST['submit'])) {
	...
	update('penumpang', $data, "idpenumpang = $_GET[idpenumpang]");
}

$penumpang = get_where("penumpang", "idpenumpang = '$_GET[idpenumpang]'");
				...
				<div class="card-body">
					<form action="#" method="post" enctype="multipart/form-data">
						<div class="mb-3">
							...
							<input type="text" class="form-control" id="idpenumpang" name="idpenumpang" value="<?= $penumpang['idpenumpang'] ?>" required>
						</div>

						<div class="mb-3">
							<label for="nama" class="form-label">Nama</label>
							<input type="text" class="form-control" id="nama" name="nama" value="<?= $penumpang['nama'] ?>" required>
						</div>

						<div class="mb-3">
							<label for="notelp" class="form-label">No.Telp</label>
							<input type="text" class="form-control" id="notelp" name="notelp" value="<?= $penumpang['notelp'] ?>" required>
						</div>

						<div class="mb-3">
							<label for="email" class="form-label">Email</label>
							<input type="text" class="form-control" id="email" name="email" value="<?= $penumpang['email'] ?>" required>
						</div>

						<div class="mb-3">
							<label for="tgl" class="form-label">Tanggal</label>
							<input type="date" class="form-control" id="tgl" name="tgl" value="<?= $penumpang['tgl'] ?>" required>
						</div>
							...
							<div class="col-md-6">
								...
								<select class="form-select" id="nmbandara" name="nmbandara" required>
									...
									<?php foreach ($bandara as $bnd) : ?>
										<option <?= $penumpang['nmbandara'] == $bnd['nmbandara'] ? 'selected' : '' ?> value="<?= $bnd['nmbandara'] ?>" data-kota="<?= $bnd['kota'] ?>"><?= $bnd['nmbandara'] ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							<div class="col-md-6">
							...
							<div class="col-md-6">
								...
								<select class="form-select" id="nmkategori" name="nmkategori" required>
									...
									<?php foreach ($kategori as $kat) : ?>
										<option <?= $penumpang['nmkategori'] == $kat['nmkategori'] ? 'selected' : '' ?> value="<?= $kat['nmkategori'] ?>" data-keterangan="<?= $kat['keterangan'] ?>"><?= $kat['nmkategori'] ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" class="form-control" id="keterangan" name="keterangan" value="<?= $penumpang['keterangan'] ?>" required>
								</div>
							</div>
						</div>
						<div class="mb-3">
							<label for="deskripsi" class="form-label">Deskripsi Hazard</label>
							<textarea class="form-control" id="deskripsi" name="deskripsi" required><?= $penumpang['deskripsi'] ?></textarea>
						</div>
						<div class="mb-3">
